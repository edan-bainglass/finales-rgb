...
def initialize_webcam(device_number=0, brightness=0.9, contrast=0.5, warmup_time=0, verbose=False):
    if verbose:
...
def get_average_color_full(device_number=0, warmup_time=0, show_image=False, verbose=False):
    cap = initialize_webcam(device_number=device_number, warmup_time=warmup_time, verbose=verbose)
...
if __name__ == "__main__":
    ...
    DO_LIST_WEBCAMS = False # Set to True to list all available webcams; requires a few seconds
    ...
    average_color = get_average_color_full(device_number=0, show_image=True, verbose=False, warmup_time=2)
